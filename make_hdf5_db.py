...
def main(video_dir, output_dir, num_jobs=1, fps=25, tmp_dir='/tmp/kinetics/'):
    ...
    for k in range(10):
        for vid in videos[:1]:
            ...
            scale_format = "{:d}*{:d}".format(int(256*height/min_dim), int(256*width/min_dim))
            (height,width) = int(256*height/min_dim), int(256*width/min_dim)
            out, _ = ffmpeg.input(video_file,).output('pipe:', format='rawvideo', pix_fmt='rgb24',r=fps, s=scale_format).run(capture_stdout=True)
...
if __name__ == '__main__':
    ...
    p.add_argument('--fps', type=int, default=25, help='Frame rate at which videos to be extracted')
