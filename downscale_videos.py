...
def convert(videoname, video_dir, output_dir, fps):
    ...
    command = 'ffmpeg -loglevel panic -i {} -c:v libx264 -preset ultrafast -filter:v scale="trunc(oh*a/2)*2:256" -q:v 1 -r {} -c:a copy {}'.format(video_file, fps, downsampled_video_file)
